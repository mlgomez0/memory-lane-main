...
const MemoryModal: React.FC<MemoryModalProps> = ({ memory }) => {
  const uploadFileIntruction =
    'Drag & drop an image here, or click to select one'
  const fileUploadButtonMessage = memory ? 'Update Image' : 'Upload Image'
  const [uploadMessage, setUploadMessage] = useState(
    memory?.imagename
      ? generateFileMessage(memory.imagename)
      : uploadFileIntruction,
  )
  ...
  const initialFormData = memory ? { ...memory } : initialData
  ...
  const handleFileUpload = (uploadedFile: File) => {
    ...
    const message = updatedValue
      ? generateFileMessage(updatedValue?.name)
      : uploadFileIntruction
  ...
  const validateFileFormat = (file: File | null) => {
    if (!file || !file.type.startsWith('image/')) {
      setError((prevError) => ({
        ...prevError,
        image: 'Error: Please upload a valid image file.',
      }))
      return null
    }
      ...
      <Dialog open={openModal} onClose={handleClose}>
        ...
        <DialogContent>
          <form onSubmit={handleSubmit}>
            ...
            <Typography
              variant='caption'
              color={formData.name.length === 100 ? 'error' : 'textSecondary'}
            >
            ...
            />
            {error.image && (
              <StyledFormHelperText>{error.image}</StyledFormHelperText>
            )}
            <DialogActions>
